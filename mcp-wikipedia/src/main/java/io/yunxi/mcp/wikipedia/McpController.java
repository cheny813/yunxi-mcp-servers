...
import io.yunxi.mcp.wikipedia.tool.WikipediaTools;
 ...
 * MCP Wikipedia 控制器
 * <p>
 * 提供 Wikipedia 操作的 MCP 端点，支持 HTTP 和 SSE 两种调用模式。
 * </p>
 *
 * <h3>支持的调用模式</h3>
 * <ul>
 ...
 * <h3>提供的工具</h3>
 * <ul>
 * <li>{@code wikipedia_search} - 搜索 Wikipedia 文章</li>
 * <li>{@code wikipedia_summary} - 获取 Wikipedia 文章摘要</li>
 * </ul>
 *
 * <h3>测试端点</h3>
 * <ul>
...
public class McpController {
     ...
     * 构造函数
     * <p>
     * 初始化 HTTP 和 SSE 端点，并注册 Wikipedia 操作工具。
     * </p>
     */
    public McpController() {
        log.info("初始化 MCP Wikipedia 控制器...");

        // 初始化 HTTP 和 SSE 端点
        this.httpEndpoint = new McpHttpEndpoint("yunxi-mcp-wikipedia", "1.0.0");
        this.sseEndpoint = new McpSseEndpoint("yunxi-mcp-wikipedia", "1.0.0");

        // 注册 Wikipedia 操作工具到 HTTP 端点
        httpEndpoint.registerTool(new WikipediaTools.SearchTool());
        httpEndpoint.registerTool(new WikipediaTools.SummaryTool());

        // 注册 Wikipedia 操作工具到 SSE 端点
        sseEndpoint.registerTool(new WikipediaTools.SearchTool());
        sseEndpoint.registerTool(new WikipediaTools.SummaryTool());

        log.info("MCP Wikipedia 控制器初始化完成，已注册 {} 个工具: wikipedia_search, wikipedia_summary", 2);
    }
