...
@RestController
@RequestMapping("/mcp")
public class PptxMcpController {

    private final McpHttpEndpoint httpEndpoint;
    private final McpSseEndpoint sseEndpoint;
    private final ExecutorService executor = Executors.newCachedThreadPool();
    private final PptxTools.PptxTool pptxTool;

    public PptxMcpController(PptxTools pptxTools) {
        log.info("初始化 MCP PPTX 控制器...");

        this.httpEndpoint = new McpHttpEndpoint("yunxi-mcp-pptx", "1.0.0");
        this.sseEndpoint = new McpSseEndpoint("yunxi-mcp-pptx", "1.0.0");

        this.pptxTool = new PptxTools.PptxTool(pptxTools);

        httpEndpoint.registerTool(pptxTool);
        sseEndpoint.registerTool(pptxTool);

        log.info("MCP PPTX 控制器初始化完成，已注册工具: pptx_operation");
    }
