...
import io.yunxi.mcp.logging.tool.ListLogFilesTool;
import io.yunxi.mcp.logging.tool.QueryLogTool;
...
public class McpController {
    ...
    @Autowired
    public McpController(LogQueryService logQueryService) {
        log.info("初始化 MCP Logging 控制器...");

        this.httpEndpoint = new McpHttpEndpoint("yunxi-mcp-logging", "1.0.0");
        this.sseEndpoint = new McpSseEndpoint("yunxi-mcp-logging", "1.0.0");

        // 注册工具
        httpEndpoint.registerTool(new QueryLogTool(logQueryService));
        httpEndpoint.registerTool(new ListLogFilesTool(logQueryService));

        sseEndpoint.registerTool(new QueryLogTool(logQueryService));
        sseEndpoint.registerTool(new ListLogFilesTool(logQueryService));

        log.info("MCP Logging 控制器初始化完成");
    }
